...
async def main():
    ...
    # # time.sleep(2)
    # await page.type('#nameNormal', 'yyy')
    # await page.type('#pwdNormal', 'XXX')
    # time.sleep(5)
    # 点击下一步
    await page.type('#searchkey', '行政处罚')
    print('searching')
    await page.click('input.index-searchbtn')
    time.sleep(5)
    print('looking for search results')
    df = await page.querySelectorAll('#search-result')
    print('printing results')
    print(df.toString())
    await page.querySelector('ul.pagination')
